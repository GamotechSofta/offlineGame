...
const PlayerDetail = () => {
        ...
        return (
            <AdminLayout onLogout={handleLogout} title="Player">
                <div className="flex flex-col items-center justify-center min-h-[40vh]">
                    ...
                    <Link to="/all-users" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold">
    ...
    return (
        <AdminLayout onLogout={handleLogout} title="Player">
            ...
            {/* Breadcrumb */}
            <div className="mb-4">
                <Link to="/all-users" className="text-slate-500 hover:text-blue-600 text-sm inline-flex items-center gap-1 mb-2">
                    <FaArrowLeft className="w-4 h-4" /> All Players
                </Link>
                <h1 className="text-2xl sm:text-3xl font-bold">Player <span className="text-slate-500 font-normal">» {player.username}</span></h1>
            </div>

            {/* Player info card - responsive, no overflow */}
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden mb-6 min-w-0">
                <div className="px-4 sm:px-6 py-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
                    <h2 className="text-lg font-semibold text-blue-600">Player Information</h2>
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold bg-slate-200 hover:bg-red-600 text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Delete player"
                        >
                        ...
                        <Link
                            ...
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                                Array.isArray(player.loginDevices) && player.loginDevices.length > 1
                                    ...
                                    : 'bg-slate-100 border border-slate-300 text-slate-700 hover:bg-slate-200 hover:border-blue-500/30 hover:text-blue-600'
                            }`}
                            title="Devices used"
                        >
                            Devices used
                            {Array.isArray(player.loginDevices) && player.loginDevices.length > 0 && (
                                <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${
                                    player.loginDevices.length > 1 ? 'bg-red-800 text-red-600' : 'bg-slate-200 text-slate-600'
                                }`}>
                ...
                <div className="p-4 sm:p-6 min-w-0">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 text-sm">
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">UserID</p>
                            <p className="text-slate-800 font-mono truncate" title={player.username}>{player.username}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">First Name</p>
                            <p className="text-slate-800 truncate">{player.username || '—'}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Last Name</p>
                            <p className="text-slate-800">—</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Email</p>
                            <p className="text-slate-800 truncate" title={player.email}>{player.email || '—'}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Name</p>
                            <p className="text-slate-800 truncate">{player.username}</p>
                        </div>
                        <div className="min-w-0 col-span-2 sm:col-span-1">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Id</p>
                            <p className="text-slate-600 font-mono text-xs truncate break-all" title={player._id}>{player._id}</p>
                        </div>
                        <div className="min-w-0 col-span-2 sm:col-span-1">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Device ID</p>
                            <p className="text-slate-600 font-mono text-xs truncate break-all" title={player.lastLoginDeviceId || ''}>{player.lastLoginDeviceId || '—'}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">IP Address</p>
                            <p className="text-slate-600 font-mono text-xs truncate" title={player.lastLoginIp || ''}>{formatIpDisplay(player.lastLoginIp)}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Status</p>
                            <span className={`inline-flex px-2.5 py-1 rounded-md text-xs font-medium ${player.isActive !== false ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-red-50 text-red-500 border border-red-200'}`}>
                                {player.isActive !== false ? 'ALLOW' : 'SUSPENDED'}
                            </span>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Balance</p>
                            <p className="text-green-600 font-mono font-semibold">{player.walletBalance ?? 0}</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Exchange Balance</p>
                            <p className="text-slate-600">0</p>
                        </div>
                        <div className="min-w-0">
                            <p className="text-slate-400 uppercase tracking-wider text-xs">Bonus Balance</p>
                            <p className="text-slate-600">0</p>
                        </div>
                    </div>
                </div>
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2">
                <span className="text-slate-500 text-sm">Date range:</span>
                    <div className="relative" ref={dropdownRef}>
                        <button
                            type="button"
                            onClick={() => setCalendarOpen((o) => !o)}
                            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-sm text-slate-700"
                        >
                            <FaCalendarAlt className="w-4 h-4 text-blue-600" />
                            {statementFrom && statementTo ? formatDateRange(statementFrom, statementTo) : 'Select Date'}
                        </button>
                        {calendarOpen && (
                            <div className="absolute left-0 top-full mt-2 py-3 rounded-xl bg-white border border-slate-300 shadow-xl z-50 flex flex-col sm:flex-row gap-4 max-w-[100vw]">
                                <div className="min-w-0 sm:min-w-[200px] py-1">
                                    {STATEMENT_PRESETS.map((p) => (
                                        ...
                                        >
                                            {statementPreset === p.id ? <span className="text-blue-600">●</span> : <span className="w-2" />}
                                            {p.label}
                                        </button>
                                    ))}
                                </div>
                                <div className="border-t sm:border-t-0 sm:border-l border-slate-300 pt-3 sm:pt-0 sm:pl-4 pr-4 min-w-0 sm:min-w-[200px]">
                                    <div className="text-xs text-slate-500 uppercase tracking-wider mb-2">Custom Date Range</div>
                                    <div className="space-y-2">
                                        <div>
                                            <label className="block text-xs text-slate-500 mb-1">From</label>
                                            <input type="date" value={statementFrom} onChange={(e) => setStatementFrom(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-sm text-slate-800" />
                                        </div>
                                        <div>
                                            <label className="block text-xs text-slate-500 mb-1">To</label>
                                            <input type="date" value={statementTo} onChange={(e) => setStatementTo(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-sm text-slate-800" />
                                        </div>
                                        <button type="button" onClick={handleDateApply} className="w-full py-2 rounded-lg bg-blue-600 text-white font-semibold text-sm">
                                            Apply
                                        </button>
                                    </div>
                                </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4 border-b border-slate-200 pb-2">
                {TABS.map((tab) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg font-semibold text-sm transition-colors ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                    >
                        {tab.label}
                    </button>
                ))}
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden min-h-[200px] min-w-0 max-w-full">
                {activeTab === 'statement' && (
                    <>
                        {loadingTab ? (
                            <div className="p-8 text-center text-slate-500">Loading...</div>
                        ) : statementData.length === 0 ? (
                            <div className="p-8 text-center text-slate-400">No transactions in this period.</div>
                        ) : (
                            <>
                                {/* Statement: cards with ALL detail - no side scroll */}
                                <div className="divide-y divide-slate-200">
                                    {statementData.map((row, i) => (
                                        <div key={i} className="p-4 sm:p-5 hover:bg-slate-50">
                                            <div className="flex flex-wrap items-center justify-between gap-2 mb-3 pb-2 border-b border-slate-200">
                                                <span className="text-blue-600 font-mono font-medium">{row.name}</span>
                                                <span className={`px-2 py-0.5 rounded text-xs font-medium ${row.status === 'WIN' || row.status === 'CREDIT' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>{row.status}</span>
                                            </div>
                                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 text-xs">
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Type</p><p className="text-slate-700 truncate">{row.type}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Name</p><p className="text-blue-600 font-mono truncate">{row.name}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Status</p><p className="text-slate-700">{row.status}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Last Balance</p><p className="text-slate-600 font-mono">{row.lastBalance}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Credited</p><p className="text-green-600 font-mono">{row.credited || '—'}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Debited</p><p className="text-red-500 font-mono">{row.debited || '—'}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Running Balance</p><p className="text-slate-800 font-mono font-medium">{row.runningBalance}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Last Bonus Balance</p><p className="text-slate-500 font-mono">{row.lastBonusBalance ?? 0}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Running Bonus Balance</p><p className="text-slate-500 font-mono">{row.runningBonusBalance ?? 0}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Last Exchange Balance</p><p className="text-slate-500 font-mono">{row.lastExchangeBalance ?? 0}</p></div>
                                                <div className="min-w-0"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Running Exchange Balance</p><p className="text-slate-500 font-mono">{row.runningExchangeBalance ?? 0}</p></div>
                                                <div className="min-w-0 col-span-2 sm:col-span-1"><p className="text-slate-400 uppercase tracking-wider mb-0.5">Updated</p><p className="text-slate-500">{row.date.toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}</p></div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                {activeTab === 'wallet' && (
                    <>
                        {loadingTab ? (
                            ...
                            <div className="p-8 text-center text-slate-400">No wallet transactions.</div>
                        ) : (
                            <div className="divide-y divide-slate-200 min-w-0">
                                {walletTx.map((t) => (
                                    <div key={t._id} className="p-4 hover:bg-slate-50 flex flex-wrap items-center justify-between gap-3">
                                        <div className="min-w-0 flex-1">
                                            <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium mr-2 ${t.type === 'credit' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>{t.type}</span>
                                            <span className="text-slate-700 text-sm break-words">{t.description || '—'}</span>
                                        </div>
                                        <div className="flex items-center gap-3 shrink-0">
                                            <span className="font-mono font-medium text-sm">{t.type === 'credit' ? '+' : '-'}{formatCurrency(t.amount)}</span>
                                            <span className="text-slate-500 text-xs">{new Date(t.createdAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {activeTab === 'bets' && (
                    <>
                        {loadingTab ? (
                            ...
                            <div className="p-8 text-center text-slate-400">No bets.</div>
                        ) : (
                            <div className="divide-y divide-slate-200 min-w-0">
                                {bets.map((b) => (
                                    <div key={b._id} className="p-4 hover:bg-slate-50">
                                        <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                                            <span className="text-blue-600 font-mono font-medium">{b.betNumber}</span>
                                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${b.status === 'won' ? 'bg-green-50 text-green-600' : b.status === 'lost' ? 'bg-red-50 text-red-500' : 'bg-slate-200 text-slate-600'}`}>{b.status}</span>
                                        </div>
                                        <p className="text-slate-500 text-xs mb-2">{b.marketId?.marketName || '—'} · {b.betType || '—'}</p>
                                        <div className="flex flex-wrap gap-4 text-sm">
                                            <span><span className="text-slate-400">Amount</span> <span className="text-slate-800 font-mono">{formatCurrency(b.amount)}</span></span>
                                            <span><span className="text-slate-400">Payout</span> <span className="text-green-600 font-mono">{formatCurrency(b.payout)}</span></span>
                                            <span className="text-slate-500 text-xs">{new Date(b.createdAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                {activeTab === 'profile' && (
                    <div className="p-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                            <div><p className="text-slate-400 text-sm">Name</p><p className="text-slate-800">{player.username}</p></div>
                            <div><p className="text-slate-400 text-sm">Email</p><p className="text-slate-800">{player.email}</p></div>
                            <div><p className="text-slate-400 text-sm">Phone</p><p className="text-slate-800">{player.phone || '—'}</p></div>
                            <div><p className="text-slate-400 text-sm">Role</p><p className="text-slate-800 capitalize">{player.role || 'Player'}</p></div>
                            <div><p className="text-slate-400 text-sm">Source</p><p className="text-slate-800">{player.source === 'bookie' ? 'Bookie' : 'Super Admin'}</p></div>
                            <div><p className="text-slate-400 text-sm">Created</p><p className="text-slate-800">{player.createdAt ? new Date(player.createdAt).toLocaleString('en-IN') : '—'}</p></div>
                        </div>
                    </div>
                )}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/30">
                    <div className="bg-white rounded-xl border border-slate-300 shadow-xl w-full max-w-md">
                        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
                            <h3 className="text-lg font-semibold text-blue-600">Edit Wallet</h3>
                            <button type="button" onClick={() => setWalletModalOpen(false)} className="text-slate-500 hover:text-slate-900 p-1">×</button>
                        </div>
                        <div className="p-4 space-y-4">
                            <div className="rounded-lg bg-slate-50 px-3 py-2">
                                <p className="text-slate-500 text-xs uppercase tracking-wider">Current Balance</p>
                                <p className="text-green-600 font-mono font-bold text-xl">{formatCurrency(player?.walletBalance ?? 0)}</p>
                            </div>
                            {walletActionError && (
                                <div className="rounded-lg bg-red-50 border border-red-200 text-red-600 text-sm px-3 py-2">{walletActionError}</div>
                            )}
                            <div>
                                <p className="text-slate-500 text-sm mb-2">Add (Credit) or Deduct (Debit)</p>
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-slate-800 placeholder-slate-400"
                                    />
                            ...
                            <div>
                                <p className="text-slate-500 text-sm mb-2">Set balance to (exact value)</p>
                                <div className="flex gap-2">
                                    <input
                                        type="number"
                                        min="0"
                                        ...
                                        className="flex-1 px-3 py-2 rounded-lg bg-slate-100 border border-slate-300 text-slate-800 placeholder-slate-400"
                                    />
                                    <button type="button" onClick={handleWalletSetBalance} disabled={walletActionLoading} className="px-4 py-2 rounded-lg bg-blue-700 hover:bg-blue-600 text-white font-semibold disabled:opacity-50">Set</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-slate-300 shadow-xl w-full max-w-md">
                        ...
                        <form onSubmit={handlePasswordSubmit} className="p-4 space-y-4">
                            ...
                            <div className="flex gap-2 justify-end">
                                ...
                                <button type="submit" disabled={togglingStatus || deletingPlayer} className="px-4 py-2 rounded-lg bg-blue-700 hover:bg-blue-600 text-white font-semibold disabled:opacity-50">
