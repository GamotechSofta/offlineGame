...
const UPLOAD_BASE_URL = (import.meta.env.VITE_API_BASE_URL || 'http://localhost:5050/api').replace(/\/api\/?$/, '') || 'http://localhost:5050';
        ...
        <AdminLayout onLogout={handleLogout} title="Help Desk">
                    ...
                    <div className="bg-white rounded-lg p-4 mb-4 sm:mb-6 flex flex-wrap gap-3 items-center">
                        <select
                            ...
                            className="px-4 py-2 bg-slate-100 border border-slate-300 rounded-lg text-slate-800"
                        >
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-slate-200 hover:bg-slate-300 border border-gray-500 rounded-lg text-slate-800 text-sm font-medium"
                        >
                            Clear filter
                        </button>
                    </div>
                                    ...
                                    {tickets.map((ticket) => (
                                        <div
                                            ...
                                            className={`p-4 cursor-pointer hover:bg-slate-100 ${
                                                selectedTicket?._id === ticket._id ? 'bg-slate-100' : ''
                                            }`}
                                        >
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <span className={`px-2 py-1 rounded text-xs ${
                                                    ...
                                                    ticket.status === 'in-progress' ? 'bg-blue-700' :
                                                    ticket.status === 'closed' ? 'bg-slate-200' :
                                                    'bg-blue-600'
                                                }`}>
                                                    {ticket.status}
                                                </span>
                                ...
                                <div>
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <span className={`px-3 py-1 rounded text-sm ${
                                            ...
                                            selectedTicket.status === 'in-progress' ? 'bg-blue-700' :
                                            selectedTicket.status === 'closed' ? 'bg-slate-200' :
                                            'bg-blue-600'
                                        }`}>
                                    ...
                                    <div className="mb-4">
                                        <p className="text-slate-500 text-sm mb-1">Player</p>
                                    ...
                                    {selectedTicket.screenshots && selectedTicket.screenshots.length > 0 && (
                                        <div className="mb-4">
                                            <p className="text-slate-500 text-sm mb-2">Screenshots (click to open full)</p>
                                            <div className="grid grid-cols-2 gap-2">
                                                {selectedTicket.screenshots.map((screenshot, index) => (
                                                    <button
                                                        ...
                                                        className="w-full h-32 rounded border border-slate-200 overflow-hidden focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                                    >
                                                        <img
                                    ...
                                    <div className="flex gap-2 mt-6">
                                        {selectedTicket.status === 'open' && (
                                            <>
                                                <button
                                                    ...
                                                    className="px-4 py-2 bg-blue-700 hover:bg-yellow-700 rounded"
                                                >
                                                    Mark In Progress
                                                </button>
                                                <button
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2 bg-slate-200 hover:bg-slate-100 rounded"
                                        >
                                            Close Ticket
                                        </button>
                                    </div>
                    ...
                    {fullScreenImage && (
                        ...
                        >
                            <button
                                type="button"
                                onClick={() => setFullScreenImage(null)}
                                className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-2xl leading-none flex items-center justify-center"
                                aria-label="Close"
                            >
                                ×
                            </button>
