...
const Reports = () => {
    ...
    return (
        <AdminLayout onLogout={handleLogout} title="Reports">
            <div className="space-y-6 print:hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Financial and betting summary for the selected period</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5">
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                        <FaCalendarAlt className="w-5 h-5 text-blue-600 shrink-0" />
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <input
                            ...
                            className="px-3 py-2 bg-slate-100 border border-slate-300 rounded-lg text-slate-800 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                        ...
                        <button
                            ...
                            onClick={fetchReport}
                            disabled={loading}
                            className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-lg transition-colors disabled:opacity-50"
                        >
                            <FaSyncAlt className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
                            Refresh
                        </button>
                        <span className="text-slate-400 text-sm">{formatRangeLabel(dateRange.startDate, dateRange.endDate)}</span>
                    </div>
                </div>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div key={i} className="bg-white rounded-xl h-28 animate-pulse border border-slate-200" />
                        ))}
                    </div>
                ) : report ? (
                    <>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div className="bg-gradient-to-br from-red-500/10 to-red-600/5 rounded-xl p-5 border border-red-500/30">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-slate-500">Total Payouts</p>
                            ...
                            <div className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 rounded-xl p-5 border border-blue-600/30">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-slate-500">Net Profit</p>
                                        <p className="text-2xl font-bold text-blue-600 mt-1">{formatCurrency(report.netProfit)}</p>
                                    </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-5">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Total Bets</p>
                                    <p className="text-xl font-bold text-slate-800 mt-1">{formatNumber(report.totalBets)}</p>
                                </div>
                                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Active Players</p>
                                    <p className="text-xl font-bold text-slate-800 mt-1">{formatNumber(report.activeUsers)}</p>
                                </div>
                                <div className="bg-slate-50 rounded-lg p-4 border border-green-500/20">
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Winning Bets</p>
                                    <p className="text-xl font-bold text-green-600 mt-1">{formatNumber(report.winningBets)}</p>
                                </div>
                                <div className="bg-slate-50 rounded-lg p-4 border border-red-500/20">
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Losing Bets</p>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-200">
                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-lg">
                                    ...
                                    <span className="text-lg font-bold text-slate-800">{report.winRate}%</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-5">
                            <h2 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                                <FaFileExport className="w-5 h-5 text-blue-600" />
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {RELATED_LINKS.map((item) => (
                                    <Link
                                        key={item.to}
                                        to={item.to}
                                        className="flex items-start gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200 hover:border-blue-300 hover:bg-slate-100 transition-all group"
                                    >
                                        <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center shrink-0 group-hover:bg-blue-50 transition-colors">
                                            <item.icon className="w-5 h-5 text-blue-600" />
                                        </div>
                                        <div className="min-w-0">
                                            <p className="font-semibold text-slate-800 group-hover:text-blue-600 transition-colors">{item.label}</p>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-200 flex flex-wrap gap-3">
                                <button
                                    type="button"
                                    onClick={handlePrint}
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-lg text-sm font-medium transition-colors print:hidden"
                                >
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                        ...
                        <p className="text-slate-500 text-lg">No report data available for this period</p>
                        <p className="text-slate-400 text-sm mt-2">Try a different date range or refresh the page</p>
                        <button
                            type="button"
                            onClick={fetchReport}
                            className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-lg transition-colors"
                        >
                            Refresh
                        </button>
                    </div>
            ...
            {/* Print-only summary */}
            {report && (
                <div className="hidden print:block mt-8 p-6 bg-white text-white rounded-lg">
                    <h2 className="text-xl font-bold mb-4">Report Summary</h2>
                    <p className="text-sm text-slate-500 mb-4">{formatRangeLabel(dateRange.startDate, dateRange.endDate)}</p>
