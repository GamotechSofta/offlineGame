        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            {markets.map((market) => {
                ...
                return (
                    <div
                        key={market._id}
                        className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5 lg:p-6 hover:border-blue-500/30 transition-colors min-w-0 overflow-hidden"
                    >
                        {/* Status Badge */}
                        <div className={`${status.color} text-slate-800 text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full inline-block mb-3 sm:mb-4`}>
                        ...
                        {/* Market Info */}
                        <h3 className="text-base sm:text-lg lg:text-xl font-bold text-slate-800 mb-2 truncate" title={market.marketName}>{market.marketName}</h3>
                        <div className="space-y-1.5 sm:space-y-2 mb-4 text-xs sm:text-sm text-slate-600 min-w-0">
                            ...
                            {market.winNumber && (
                                <p><span className="font-semibold">Win Number:</span> <span className="text-green-600 font-mono">{market.winNumber}</span></p>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                            <button
                                ...
                                className="px-2 sm:px-3 py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-lg text-xs sm:text-sm font-semibold min-h-[40px] sm:min-h-0"
                            >
                                View
                            </button>
                            <button
                                onClick={() => onEdit(market)}
                                className="px-2 sm:px-3 py-2 bg-blue-700 hover:bg-yellow-700 rounded-lg text-xs sm:text-sm font-semibold min-h-[40px] sm:min-h-0"
                            >
                                Edit
                            </button>
                            <button
        ...
        {showPasswordModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
                <div className="bg-white rounded-xl border border-slate-200 shadow-xl max-w-md w-full p-6">
                    <h3 className="text-lg font-bold text-blue-600 mb-2">Enter Secret Password to Delete Market</h3>
                    ...
                    <form onSubmit={handlePasswordSubmit} className="space-y-4">
                        <input
                            ...
                            autoFocus
                            className="w-full px-4 py-3 rounded-lg bg-slate-100 border border-slate-300 text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        />
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold rounded-lg border border-slate-300"
                            >
                                Cancel
                            </button>
                        </div>
