...
const MarketForm = ({ market, defaultMarketType = 'main', onClose, onSuccess, apiBaseUrl, getAuthHeaders }) => {
    ...
    const handleClose12Change = (field, value) => {
        ...
        setFormData((prev) => ({ ...prev, closingTime: newClosing }));
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/30 flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6 z-50 overflow-y-auto min-h-screen">
            <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 sm:my-auto max-h-[90vh] sm:max-h-[calc(100vh-2rem)] overflow-y-auto flex flex-col">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-slate-100 border border-slate-300 rounded-lg text-slate-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-base"
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-[1fr_auto_1fr_auto_auto] gap-1 sm:gap-2 items-center">
                                <select
                                    ...
                                    className="w-full min-w-0 px-2 sm:px-3 py-2.5 sm:py-2.5 bg-slate-100 border border-slate-300 rounded-lg text-slate-800 focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                                >
                                    {HOURS_12.map((h) => (
                                ...
                                <select
                                    ...
                                    className="w-full min-w-[4rem] px-2 sm:px-3 py-2.5 sm:py-2.5 bg-slate-100 border border-slate-300 rounded-lg text-slate-800 focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                                >
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-0 rounded-lg overflow-hidden border border-slate-300">
                                <input
                                    type="number"
                                    name="betClosureTime"
                                    ...
                                    placeholder="e.g. 300"
                                    inputMode="numeric"
                                    className="flex-1 min-w-[80px] sm:min-w-[100px] px-3 sm:px-4 py-2.5 sm:py-2.5 bg-slate-100 text-slate-800 placeholder-slate-400 text-sm sm:text-base border-0 focus:ring-2 focus:ring-blue-500 focus:ring-inset"
                                />
                                <span className="inline-flex items-center justify-center px-3 sm:px-4 py-2.5 sm:py-2.5 bg-slate-200 text-slate-600 text-xs sm:text-sm font-medium whitespace-nowrap">
                                    Seconds
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 pt-4 pb-4 sm:pb-0">
                            <button
                                type="button"
                                onClick={onClose}
                                className="flex-1 w-full sm:w-auto bg-slate-200 hover:bg-slate-100 text-slate-800 font-semibold py-2.5 sm:py-2 px-4 rounded-lg transition-colors"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                disabled={loading}
                                className="flex-1 w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 sm:py-2 px-4 rounded-lg transition-colors disabled:opacity-50"
                            >
