...
const FullSangamBid = ({ market, title }) => {
    ...
    const handleSubmitBet = async () => {
        ...
        const payload = bids.map((b) => ({ betType: 'full-sangam', betNumber: String(b?.number ?? '').trim(), amount: Number(b?.points) || 0, betOn: String(b?.type || session).toUpperCase() === 'CLOSE' ? 'close' : 'open' })).filter((b) => b.betNumber && b.amount > 0);
    ...
    return (
        <BidLayout market={market} title={title} bidsCount={bids.length} totalPoints={totalPoints} showDateSession={true} selectedDate={selectedDate} setSelectedDate={handleDateChange} extraHeader={null} session={session} setSession={setSession} sessionOptionsOverride={['OPEN']} lockSessionSelect hideFooter walletBalance={walletBefore} contentPaddingClass="pb-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex flex-row items-center gap-2">
                                ...
                                <input type="text" inputMode="numeric" value={openPana} onChange={(e) => { const next = sanitizeDigits(e.target.value, 3); setOpenPana(next); setOpenPanaInvalid(!!next && next.length === 3 && !isValidAnyPana(next)); }} onBlur={() => { if (openPana && openPana.length === 3 && !isValidAnyPana(openPana)) showWarning('Open Pana must be a valid Single / Double / Triple Pana (3 digits).'); }} placeholder="Pana" className={`flex-1 min-w-0 bg-blue-50 border border-blue-200 text-gray-900 placeholder-gray-500 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm focus:ring-2 focus:outline-none ${openPanaInvalid ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'focus:ring-blue-500 focus:border-blue-500'}`} />
                            </div>
                            <div className="flex flex-row items-center gap-2">
                                <label className="text-gray-600 text-sm font-medium shrink-0 w-40">Enter Close Pana:</label>
                                <input type="text" inputMode="numeric" value={closePana} onChange={(e) => { const prevLen = (closePana ?? '').toString().length; const next = sanitizeDigits(e.target.value, 3); setClosePana(next); setClosePanaInvalid(!!next && next.length === 3 && !isValidAnyPana(next)); if (next.length === 3 && prevLen < 3) { if (!isValidAnyPana(next)) { showWarning('Close Pana must be a valid Single / Double / Triple Pana (3 digits).'); return; } window.requestAnimationFrame(() => { pointsInputRef.current?.focus?.(); }); } }} onBlur={() => { if (closePana && closePana.length === 3 && !isValidAnyPana(closePana)) showWarning('Close Pana must be a valid Single / Double / Triple Pana (3 digits).'); }} placeholder="Pana" className={`flex-1 min-w-0 bg-blue-50 border border-blue-200 text-gray-900 placeholder-gray-500 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm focus:ring-2 focus:outline-none ${closePanaInvalid ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'focus:ring-blue-500 focus:border-blue-500'}`} />
                            </div>
                            <div className="flex flex-row items-center gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-3 md:grid-cols-1">
                            ...
                            <button type="button" onClick={openReview} disabled={!bids.length} className={`w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold py-3.5 min-h-[48px] rounded-lg shadow-md transition-all ${bids.length ? 'hover:from-blue-600 hover:to-blue-700 active:scale-[0.98]' : 'opacity-50 cursor-not-allowed'}`}>Submit Bet</button>
                        </div>
                    </div>
                    <div className="mt-10 md:mt-0">
                        <div className="grid grid-cols-[1.4fr_0.7fr_0.7fr_0.5fr] gap-2 sm:gap-3 text-center text-blue-600 font-bold text-xs sm:text-sm mb-2 px-2"><div className="truncate">Sangam</div><div className="truncate">Point</div><div className="truncate">Type</div><div className="truncate">Delete</div></div>
                        <div className="h-px bg-blue-100 w-full mb-2"></div>
                        {bids.length === 0 ? null : (
                            <div className="space-y-2">
                                {bids.map((b) => (
                                    <div key={b.id} className="grid grid-cols-[1.4fr_0.7fr_0.7fr_0.5fr] gap-2 sm:gap-3 text-center items-center py-2.5 px-3 bg-blue-50 rounded-lg border border-blue-200 text-sm">
