...
const HalfSangamBBid = ({ market, title }) => {
    ...
    const computeOpenAnkFromPana = (pana) => { const s = (pana ?? '').toString().trim(); if (!/^[0-9]{3}$/.test(s)) return ''; const sum = Number(s[0]) + Number(s[1]) + Number(s[2]); return String(sum % 10); };

    const clearAll = () => {
        setIsReviewOpen(false); setOpenAnk(''); setClosePana(''); setPoints(''); setBids([]);
    ...
    const handleAdd = () => {
        ...
        if (!/^[0-9]$/.test(derivedOpenAnk)) { showWarning('Open Ank could not be calculated. Please re-enter Close Pana.'); return; }
    ...
    return (
        <BidLayout market={market} title={title} bidsCount={bids.length} totalPoints={totalPoints} showDateSession={true} selectedDate={selectedDate} setSelectedDate={handleDateChange} session={session} setSession={setSession} sessionOptionsOverride={['CLOSE']} lockSessionSelect hideFooter walletBalance={walletBefore} contentPaddingClass="pb-[calc(7rem+env(safe-area-inset-bottom,0px))] md:pb-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex flex-row items-center gap-2">
                                <label className="text-gray-600 text-sm font-medium shrink-0 w-40">Enter Open Ank:</label>
                                <input type="text" inputMode="numeric" value={openAnk} readOnly placeholder="Ank" className="flex-1 min-w-0 bg-blue-50 border border-blue-200 text-gray-900 placeholder-gray-500 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm opacity-80 cursor-not-allowed focus:outline-none" />
                            </div>
                            <div className="flex flex-row items-center gap-2">
                                <label className="text-gray-600 text-sm font-medium shrink-0 w-40">Enter Close Pana:</label>
                                <input type="text" inputMode="numeric" value={closePana} onChange={(e) => { const prevLen = (closePana ?? '').toString().length; const next = sanitizeDigits(e.target.value, 3); setClosePana(next); setClosePanaInvalid(!!next && next.length === 3 && !isValidAnyPana(next)); setOpenAnk(computeOpenAnkFromPana(next)); if (next.length === 3 && prevLen < 3) { if (!isValidAnyPana(next)) { showWarning('Close Pana must be a valid Single / Double / Triple Pana (3 digits).'); return; } window.requestAnimationFrame(() => { pointsInputRef.current?.focus?.(); }); } }} placeholder="Pana" className={`flex-1 min-w-0 bg-blue-50 border border-blue-200 text-gray-900 placeholder-gray-500 rounded-full py-2.5 min-h-[40px] px-4 text-center text-sm focus:ring-2 focus:outline-none ${closePanaInvalid ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'focus:ring-blue-500 focus:border-blue-500'}`} />
                            </div>
                            <div className="flex flex-row items-center gap-2">
