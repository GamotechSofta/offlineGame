...
const BidLayout = ({
    ...
    return (
        <div className="min-h-screen bg-white font-sans w-full max-w-full overflow-x-hidden">
            <div
                className="bg-blue-800 border-b border-blue-700 py-2 flex items-center justify-between gap-2 sticky top-0 z-10 mt-4"
                style={{ paddingLeft: 'max(0.75rem, env(safe-area-inset-left))', paddingRight: 'max(0.75rem, env(safe-area-inset-right))' }}
            >
                <button
                    onClick={() => market ? navigate(`/bid-options/${market._id || market.id}`, { state: { market } }) : navigate(-1)}
                    className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center bg-blue-700 hover:bg-blue-600 text-white rounded-full active:scale-95 transition-colors touch-manipulation"
                    aria-label="Back"
                >
                ...
                <h1 className="text-xs sm:text-base md:text-lg font-bold uppercase tracking-wide truncate flex-1 text-center mx-1 text-white min-w-0">
                    {(market?.marketName || market?.gameName) ? `${market.marketName || market.gameName} - ${title}` : title}
                </h1>
                <div className="bg-blue-600 text-white px-2 sm:px-3 py-1 rounded-full flex items-center gap-1.5 text-[11px] sm:text-sm font-bold shadow-md shrink-0">
                    <div className="w-5 h-5 bg-white rounded flex items-center justify-center text-blue-600 text-xs font-bold">₹</div>
                    {wallet.toFixed(1)}
                </div>
            </div>

            {!bettingAllowed && bettingMessage && (
                <div className="mx-3 sm:mx-6 mt-2 p-3 rounded-xl bg-red-100 border border-red-300 text-red-600 text-sm font-medium flex items-center gap-2">
                ...
                >
                    <div className="relative flex-1 min-w-0 shrink overflow-hidden">
                        ...
                        <input
                            ...
                            className={`w-full pl-9 sm:pl-10 pr-3 py-2.5 min-h-[44px] h-[44px] bg-blue-50 border border-blue-200 text-gray-900 rounded-full text-xs sm:text-sm font-bold text-center focus:outline-none focus:border-blue-500 cursor-pointer truncate ${dateSessionControlClassName}`}
                    ...
                    <div className="relative flex-1 min-w-0 hidden md:block">
                        <select
                            ...
                            className={`w-full appearance-none bg-blue-50 border border-blue-200 text-gray-900 font-bold text-xs sm:text-sm py-2.5 min-h-[44px] h-[44px] px-4 pr-8 rounded-full text-center focus:outline-none focus:border-blue-500 ${(lockSessionSelect || (isToday && isRunning)) ? 'opacity-80 cursor-not-allowed' : ''} ${dateSessionControlClassName}`}
                        >
                ...
                <div className="flex justify-center md:justify-center">
                    <div
                        className={`w-full max-w-sm md:max-w-md rounded-2xl flex flex-col sm:flex-row items-center gap-4 sm:gap-6 ${
                            showFooterStats
                                ? 'bg-white/95 backdrop-blur-sm border border-blue-200 shadow-xl shadow-blue-300/30 px-4 py-4'
                        ...
                        {showFooterStats && (
                            <div className="flex items-center gap-6 sm:gap-8 shrink-0">
                                <div className="text-center">
                                    ...
                                    <div className="text-base sm:text-lg font-bold text-blue-600">{bidsCount}</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-[10px] sm:text-xs text-gray-600 uppercase tracking-wider">Points</div>
                                    <div className="text-base sm:text-lg font-bold text-blue-600">{totalPoints}</div>
                                </div>
                            </div>
                        )}
                        <button
