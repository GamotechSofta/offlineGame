                ...
                <div className="md:grid md:grid-cols-2 md:gap-12 md:items-start">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-3">
                            {/* First field (depends on flip state) */}
                            {!flipped ? (
                            ...
                            {/* Flip button — always in between */}
                            <div className="flex justify-center">
                                <button
                                    type="button"
                                    onClick={() => setFlipped((f) => !f)}
                                    className="w-9 h-9 rounded-full bg-[#202124] border border-[#d4af37]/30 hover:border-[#d4af37]/60 hover:bg-[#d4af37]/10 flex items-center justify-center transition-all active:scale-90"
                                    title="Flip fields"
                                >
                                    <svg className={`w-5 h-5 text-[#d4af37] transition-transform duration-300 ${flipped ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M7 16V4m0 0L3 8m4-4l4 4m6 4v12m0 0l4-4m-4 4l-4-4" />
                                    </svg>
                                </button>
                            </div>

                            {/* Second field (depends on flip state) */}
                            {!flipped ? (
                                <div className="flex flex-row items-center gap-2">
                                    <label className="text-gray-400 text-sm font-medium shrink-0 w-40">Enter Close Ank:</label>
                                    <input
